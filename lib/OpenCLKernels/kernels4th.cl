...
//__attribute__((reqd_work_group_size(256,1,1)))
//__attribute__((work_group_size_hint(256,1,1)))
void dev_evaluate_gravity_reduce(
                                     const          int        nj_total, 
                                     const          int        nj,
                                     const          int        ni_offset,    
                                     const          int        ni_total,
...
#pragma unroll 32
  ...
  __local int    *shared_nid = (__local int*  )&shared_ofs[Dim];
  ...
  if (ty == 0) 
  {
    __global int *atomicVal = (__global int*)&result_i[3*ni_total+1];
    if(threadIdx_x == 0)
    {
    ...
    if(ds2_min2.x <  temp2.y.y)
    {
      temp2.y.y = ds2_min2.x;
      temp2.y.x = ds2_min2.y;
      dsminNNB[tx+ni_offset] = temp2.x;
    }
